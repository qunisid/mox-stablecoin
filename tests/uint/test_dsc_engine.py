...
def test_reverts_if_collateral_zero(some_user, weth, dsce):
    with boa.env.prank(some_user):
        ...
        with pytest.raises(boa.BoaError):  # 或者用 Exception 也行
            dsce.deposit_collateral(weth.address, 0)


def test_can_deposite_collateral(some_user, weth, dsce):
    with boa.env.prank(some_user):
        weth.approve(dsce.address, COLLATERAL_AMOUNT)
        dsce.deposit_collateral(weth.address, COLLATERAL_AMOUNT)

    deposited_amount = dsce.user_to_token_amount_deposited(some_user, weth.address)
    print("COLLATERAL_AMOUNT:", COLLATERAL_AMOUNT)
    print("dsce.address:", weth.address)
    assert deposited_amount == COLLATERAL_AMOUNT


def test_reverts_if_price_feed_is_zero(some_user, weth, dsce):
    with boa.env.prank(some_user):
        weth.approve(dsce.address, COLLATERAL_AMOUNT)
        with pytest.raises(boa.BoaError):
            dsce.deposit_collateral(
                "0x0000000000000000000000000000000000000000", COLLATERAL_AMOUNT
            )
            # dsce.deposit_collateral(empty(address), COLLATERAL_AMOUNT)
...
def test_get_usd_value(dsce, weth):
    expected_usd = 2000
    actual_usd = dsce.get_usd_value(weth, 1)
    print("actual_usd:", actual_usd)
...
def test_reverts_if_minted_dsc_breaks_health_factor(dsce, weth, eth_usd, some_user):
    ...
    with boa.env.prank(some_user):
        weth.approve(dsce, COLLATERAL_AMOUNT)
        health_factor = dsce.calculate_health_factor(
...
def test_can_minted_dsc_if_breaks_health_factor_is_ok(dsce, weth, eth_usd, some_user):
    price = eth_usd.latestRoundData()[1]
    amount_to_mint = (
        (COLLATERAL_AMOUNT * (price * dsce.ADDITIONAL_FEED_PRECISION()))
        // dsce.PRECISION()
    ...
    with boa.env.prank(some_user):
        ...
        print("health_factor", from_wei(health_factor, "ether"))
...
# ------------------------------------------------------------------
def test_can_redeem_collateral(some_user, weth, dsce, dsc):
    with boa.env.prank(some_user):
        weth.approve(dsce, COLLATERAL_AMOUNT)
...
def test_properly_reports_health_factor(dsce_minted, some_user):
    ...
    # factor = from_wei(actual_health_factor)
    # print(factor, "ether")
    breakpoint()
...
def test_health_factor_can_go_below_one(dsce_minted, eth_usd, some_user):
    ...
    user_health_factor = from_wei(dsce_minted.health_factor(some_user), "ether")
    breakpoint()
...
def test_liquidator_takes_on_users_debt(dsce_liquidated, liquidator):
    ...
    liquidator = from_wei(liquidator_dsc_minted, "ether")
    breakpoint()
